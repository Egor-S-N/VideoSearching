    ...
    def show_video(self):
        ...
        while video_capture.isOpened():
            ...
            if len(good_matches) > 10:
                reference_points = np.float32([keypoints[m.queryIdx].pt for m in good_matches])
                current_points = np.float32([current_keypoints[m.trainIdx].pt for m in good_matches])
                H, mask = cv2.findHomography(reference_points, current_points, cv2.RANSAC, 5.0)
